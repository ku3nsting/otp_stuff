...
//********************************
//
// D E C R Y P T O R  function
//
//********************************
char* doDecryption(char message[], char key[]){
	  int i = 0;
	  char current;
	  char keyChar;
	  int encrypted;
	  
	  //iterate until you hit the end of plaintext, which is a null terminator
	  while(message[i] != '\0'){
			...
			//subtract them to get original number
			encrypted = (msg - keyInt);
			
			//if result is less than 0, add 27 to get it in rance
			if(encrypted < 0){
